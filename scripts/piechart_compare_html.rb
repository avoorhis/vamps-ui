...
myfile.puts "<script type=\"text/javascript\" src = \"scripts/rotate.js\"></script>"
...
lines.each do |row|
    ...
    items.each do |cell|
		...
		if col_counter == row_counter
		    color = black
		    tooltip = " " + '|'+ datasets[row_counter] + '|'+metric+'|00'
		    myfile.puts "<td class='same' id='#{tooltip}' bgcolor='#{color}' width='#{cell_width}' height='#{cell_height}'></td>"
		else
		    tooltip = datasets[col_counter] +'|'+ datasets[row_counter] + '|'+metric+'|'+cell
		    myfile.puts "<td class='cell' id='#{tooltip}' bgcolor='#{color}' width='#{cell_width}' height='#{cell_height}'></td>"
		end		
